...
function evaluateTumorRegionT1ImgFile4NII()
% instensity image
d=32;
len=13;
step=8;
...
% tmat=resize(single(tumorData),[64,64,64]); % sometimes, you need this
% sentence to resize the data. But I suggest not to use it.
...
%d: the patch size
function pred = cropCubicHdf5(matData,loc,label,days,subID,d,step)   
    %segPath='CT/';
    if nargin<5
        step=8;
    end
    ...
    for i=1:step:rowData-d+1%xmin,xmax
        for j=1:step:colData-d+1%ymin,ymax
            for k=1:step:lenData-d+1%zmin,zmax
                ...
                class = test_3dBrainCNN(volT1);
                
                categories(cubicCnt) = class;
            end
